...
void KeyboardInput::Update() {
	...
	// キーボードの入力状態を取得
	keyboardDevice_->GetDeviceState(sizeof(state_.first), state_.first.data());
	
}

bool KeyboardInput::IsPress(KeyId id) const {
	const auto& [first, second] = state_;
	return first[static_cast<uint8_t>(id)];
}

bool KeyboardInput::IsTrigger(KeyId id) const {
	const auto& [first, second] = state_;
	return (first[static_cast<uint8_t>(id)]) && !(second[static_cast<uint8_t>(id)]);
}
...
void MouseInput::Update() {
	...
	// マウスの入力状態を取得
	mouseDevice_->GetDeviceState(sizeof(state_.first), &state_.first);
...
Vector2i MouseInput::GetDeltaPosition() const {
	return { state_.first.lX, state_.first.lY };
}

bool MouseInput::IsPress(MouseId id) const {
	const auto& [first, second] = state_;
	return first.rgbButtons[static_cast<uint8_t>(id)];
}

bool MouseInput::IsTrigger(MouseId id) const {
	const auto& [first, second] = state_;
	return (first.rgbButtons[static_cast<uint8_t>(id)]) && !(second.rgbButtons[static_cast<uint8_t>(id)]);
}
...
float MouseInput::GetDeltaWheel() const {
	return static_cast<float>(state_.first.lZ) / WHEEL_DELTA; //!< wheelの最大値でnormalize
}

bool MouseInput::IsWheelUp() const {
	return state_.first.lZ > 0;
}
